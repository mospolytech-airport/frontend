    ...
    <div class="content">
      <div class="filters">
        ...
        <label style="position: relative">
          From
          <select class="f ilter" v-model="from" @change="selectFilter">
            <option
        ...
        <label style="position: relative">
          ...
          <span class="clear-button" v-if="to" @click="removeFilter('to')"
            >x
          </span>
        </label>
        <label style="position: relative">
          Sort by
          <select class="filter" v-model="sort">
            <option v-for="(name, key) in sortBy" :key="key" :value="name">
              {{ name }}
            </option>
          </select>
          <span class="clear-button" v-if="sort" @click="sort = sortBy[0]"
            >x
          </span>
        </label>
        <label style="position: relative">
          Outbound
          <input class="filter" v-model="outbound" placeholder="dd.mm.yyyy" />
        ...
        <label style="position: relative">
          Flight Number
          <input class="filter" v-model="flightNumber" placeholder="xxxx" />
          <span
            class="clear-button"
            v-if="flightNumber"
      ...
      <div class="buttons">
        <div style="display: flex; gap: 3rem">
          <button @click="cancelSchedule" :disabled="!selectedRow">
            Cancel Flight
          </button>
          <button @click="updateModalStatus(true)" :disabled="!selectedRow">
            Edit Flight
          </button>
        </div>
        <button @click="updateSecondModalStatus(true)">Import Changes</button>
      </div>
    </div>
    <Modal v-if="isModalOpen" @update:isOpen="updateModalStatus">
      <template #header>
        <p>Schedule edit</p>
      </template>
      <template #content>
        <p class="modal-p">Flight route</p>
        ...
        <div class="modal-edit">
          <label style="position: relative">
            Date:
            <input class="filter" v-model="editDate" placeholder="yyyy-mm-dd" />
          </label>
          <label style="position: relative">
            Time:
            <input class="filter" v-model="editTime" placeholder="hh:mm:ss" />
          </label>
          <label style="position: relative">
            Economy Price: $
            <input
      ...
      <template #footer>
        <button class="footer-button" @click="updateSchedule">Update</button>
        <button class="footer-button" @click="updateModalStatus(false)">
          Cancel
        </button>
      </template>
    </Modal>
    <Modal v-if="isSecondModalOpen" @update:isOpen="updateSecondModalStatus">
      <template #header>
      ...
      <template #content>
        ...
        <p class="modal-results">Results</p>
